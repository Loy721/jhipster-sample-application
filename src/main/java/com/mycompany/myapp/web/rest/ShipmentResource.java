...
public class ShipmentResource {
     ...
     */
    @GetMapping("")
    public List<Shipment> getAllShipments(@RequestParam(name = "eagerload", required = false, defaultValue = "true") boolean eagerload) {
        log.debug("REST request to get all Shipments");
        if (eagerload) {
            return shipmentRepository.findAllWithEagerRelationships();
        } else {
    ...
    public ResponseEntity<Shipment> getShipment(@PathVariable("id") Long id) {
        ...
        Optional<Shipment> shipment = shipmentRepository.findOneWithEagerRelationships(id);
